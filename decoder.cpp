...
void func9(string, int[], int[], int, int);
void func10(string, int[], int);
void func11(string, int, int);
void func12(string, int, int);
void func13(string, int, int);
...
int func5(int arr[], int val, int len) {
	int dist=abs(val-arr[0]);
	int index=0;
	for(int i=1; i<len;i++){
		if(abs(val-arr[i])<dist){
			dist=abs(val-arr[i]);
			index=i;
		}
	}
